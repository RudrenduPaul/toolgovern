...
| [#6171](https://github.com/microsoft/agent-framework/pull/6171) | shrutitople | **PASS (N/A -- already resolved upstream via a parallel mechanism)** | MERGED. This is Agent Framework's own native FIDES information-flow-control feature (MCP-annotation-based tool/result labeling). It is a parallel, coexisting mechanism to toolgovern's own TG08 IFC classifier (Foundation step 4), not a gap toolgovern needs to fill. No code change needed; both can run layered (toolgovern's `IfcPolicy` check in `classify()`, Agent Framework's own FIDES labels), since neither depends on the other. |
